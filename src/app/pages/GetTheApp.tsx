...
import { Download, Share, PlusSquare, MoreVertical, Smartphone, Chrome, Info, Sparkles, GraduationCap, Utensils, Users, TicketPercent, X, ArrowRight } from 'lucide-react';
...
const PWAInstructions = ({ isModal = false }) => (
    <div className={`grid ${isModal ? 'grid-cols-1 gap-12' : 'lg:grid-cols-2 gap-12'}`}>
        {/* iOS */}
    ...
    return (
        <div className="min-h-screen bg-brand-cream text-brand-dark-green font-montserrat overflow-x-hidden">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
                <div className="absolute top-[10%] left-[-10%] w-[400px] h-[400px] bg-brand-pink/30 rounded-full blur-3xl" />
                <div className="absolute bottom-[20%] right-[-5%] w-[350px] h-[350px] bg-brand-green/5 rounded-full blur-3xl" />
            </div>
            ...
            <main className="pt-32 pb-20 relative z-10">
                <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto mb-20">
                    <div className="grid lg:grid-cols-2 gap-16 items-center">
                        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                            <div className="inline-flex items-center gap-2 bg-brand-green/10 text-brand-green px-5 py-2 rounded-full font-bold mb-6 border border-brand-green/20">
                                <Sparkles size={16} /> L'assistant parental dans votre poche
                            </div>
                            <h1 className="text-6xl md:text-8xl font-londrina text-brand-green mb-8 leading-[0.9]">Tout HappyDarons en un clic</h1>
                            <p className="text-xl md:text-2xl text-brand-dark-green mb-10 leading-relaxed font-medium">Installez l'application en quelques secondes et retrouvez vos programmes partout avec vous.</p>
                            <div className="flex flex-wrap gap-5">
                                <button onClick={handleOpenApp} className="bg-brand-green text-white px-10 py-5 rounded-[2rem] font-bold text-xl hover:scale-105 transition-all flex items-center gap-3 shadow-xl shadow-brand-green/20">
                                    <Smartphone className="w-6 h-6" /> Ouvrir l'app
                                </button>
                                <a href="#instructions" className="bg-brand-pink text-brand-green border-2 border-brand-green/10 px-10 py-5 rounded-[2rem] font-bold text-xl hover:scale-105 transition-all flex items-center gap-2">Mode d'emploi</a>
                            </div>
                        </motion.div>
                ...
                <section className="bg-white py-24 px-4 sm:px-6 lg:px-8 border-y-2 border-brand-pink">
                    <div className="max-w-7xl mx-auto">
                        <div className="text-center mb-16"><h2 className="text-5xl font-londrina text-brand-green mb-6">Un contenu exceptionnel</h2></div>
                        <div className="grid md:grid-cols-2 gap-8">
                            <div className="bg-brand-green/5 p-10 rounded-[3.5rem] border-2 border-brand-pink/50">
                                <GraduationCap className="text-brand-green mb-6" size={40} />
                                <h3 className="text-3xl font-bold text-brand-green mb-4">Bibliothèque & Programmes</h3>
                                <p className="text-lg text-brand-dark-green/80 font-medium">Plus de 250 articles et programmes vidéo thématiques créés par des pros.</p>
                            </div>
                            <div className="bg-brand-pink/20 p-10 rounded-[3.5rem] border-2 border-brand-green/10">
                                <Utensils className="text-brand-green mb-6" size={40} />
                                <h3 className="text-3xl font-bold text-brand-green mb-4">Organisation Family Hub</h3>
                                <p className="text-lg text-brand-dark-green/80 font-medium">Meal Planner, Shopping List partagée et Calendrier familial centralisé.</p>
                            </div>
                        </div>
                    </div>
                </section>
